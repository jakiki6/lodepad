...
public class Lodepad implements ModInitializer {
    public static final Block LODEPAD_BLOCK  = new LodepadBlock(FabricBlockSettings.create().mapColor(MapColor.BLACK).instrument(Instrument.BASEDRUM).requiresTool().strength(Blocks.RESPAWN_ANCHOR.getHardness(), Blocks.RESPAWN_ANCHOR.getBlastResistance()).luminance(state -> state.get(LodepadBlock.ENABLED) ? 15 : 0));
